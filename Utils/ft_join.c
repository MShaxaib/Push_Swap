...
/*   Updated: 2024/01/20 12:44:44 by codespace        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    while ((argv[i][j] == ' ' )|| ((argv[i][j] >= 9 && argv[i][j] <= 13) && argv[i][j] != '\0'))
        j++;
    return(j);
}
void ft_copy(char *argv, char *list, int *argv_index, int *list_index)
{
    ...
    while(argv[arg_index] != '\0' && !(argv[arg_index] >= 9 && argv[arg_index] <= 13) && argv[arg_index] != '\0')
    {
